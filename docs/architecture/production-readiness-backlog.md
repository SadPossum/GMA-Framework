...
| Notifications and realtime streaming | Implemented | ADR 0011 and ADR 0012 add optional front-door notification contracts, a generic realtime fanout layer, a notification-to-realtime bridge, SSE and SignalR adapters, persisted notification history/read state, admin history access, a durable Notifications request event, bounded queues, fail-open delivery, metrics, architecture guards, and targeted streaming tests. Preferences, delivery receipts, retention cleanup, and multi-instance live backplanes remain future optional slices. |
| Shared access subject foundation | First slice implemented | `Gma.Framework.AccessControl` now provides backend-agnostic `AccessSubject` primitives only. Module-owned domain visibility scopes handle list/detail filtering, while simple application checks stay direct. Persisted grants, generic policy evaluation, and external policy engines remain deferred until repeated module needs prove the shape. |
| Generic access control and RBAC | Planned | Promote RBAC out of Administration-owned persistence into optional generic AccessControl infrastructure, while keeping Auth identity-only and Administration as the management surface. See [Generic Access Control And RBAC Refactor Task](generic-access-control-rbac-task.md). |
